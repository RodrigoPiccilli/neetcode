class Solution {
    ...
    private void dfs(int r, int c, int[][] heights, boolean[][] ocean) {
        ...
        if(r - 1 >= 0 && r - 1 < ROWS && heights[r - 1][c] >= heights[r][c] && !ocean[r - 1][c]) dfs(r - 1, c, heights, ocean);
        if(r + 1 >= 0 && r + 1 < ROWS && heights[r + 1][c] >= heights[r][c] && !ocean[r + 1][c]) dfs(r + 1, c, heights, ocean);
        if(c - 1 >= 0 && c - 1 < COLS && heights[r][c - 1] >= heights[r][c] && !ocean[r][c - 1]) dfs(r, c - 1, heights, ocean);
        if(c + 1 >= 0 && c + 1 < COLS && heights[r][c + 1] >= heights[r][c] && !ocean[r][c + 1]) dfs(r, c + 1, heights, ocean);
